...
f.write('/nResults file for: '+args.input)

hdf_files = []
for file in os.listdir(args.input):
    if file.endswith('.hdf5'):
        hdf_files.append(os.path.join(args.input, file))

dfs = []
for i in hdf_files:
    file_data = pd.read_hdf(i, 'table')
...
f.write('\n\nTrue: '+str((len(true)/len(all_solved))*100))
f.write('\nFalse: '+str((len(false)/len(all_solved))*100))
